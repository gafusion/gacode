...
SUBROUTINE xgrid_functions_sa
  ...
  if(alpha_quench_in.eq.0.0.and.gamma_reference_kx0(1).ne.0.0)then
     ...
     if(sat_rule_in.eq.2 .OR. sat_rule_in.eq.3 .OR. sat_rule_in.eq.4)then
...
!      write(*,*)"kx0_e = ",kx0_e,kymax_out,vzf_out
     ...
     if(sat_rule_in.eq.2 .OR. sat_rule_in.eq.3 .OR. sat_rule_in.eq.4)a0=1.6
...
!     endif
     ...
     if(units_in.eq.'GYRO')then
       ...
       if(sat_rule_in.eq.2 .OR. sat_rule_in.eq.3 .OR. sat_rule_in.eq.4)kx0 = sign_Bt_in*kx0_e*0.7/grad_r0_out**2     ! goes with xnu_model=3, the factor 0.7/grad_r0_out**2 is needed for stress_tor
 ...
 ! Nov 2019     SAT_geo0_out = 0.946/qrat_geo(0) ! normed to GASTD with CGYRO
    ...
    if(sat_rule_in.eq.2 .OR. sat_rule_in.eq.3 .OR. sat_rule_in.eq.4)SAT_geo0_out = 1.0
